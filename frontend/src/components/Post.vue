...
const checkBookmarkStatus = () => {
  ...
  return user.bookmarks.includes(props.postData._id);
};

// Initialize bookmark status on mount
onMounted(() => {
  isBookmarked.value = checkBookmarkStatus();
});
...
const toggleLike = async () => {
  ...
  try {
    // Backend routes: /:id/like (POST) and /:id/dislike (POST)
...
const toggleBookmark = async () => {
  ...
  try {
    const response = await axios.post(
      `http://localhost:3000/api/v1/post/${props.postData._id}/bookmark`,
      {},
      {
        headers: {
          Authorization: `Bearer ${sessionStorage.getItem("token")}`,
    ...
    if (response.data.success) {
      ...
      isBookmarked.value = responseData.type === "saved";

      // Update user's bookmarks in current user data
      if (responseData.type === "saved") {
        // Add post to user's bookmarks
        if (!user.bookmarks) user.bookmarks = [];
        if (!user.bookmarks.includes(props.postData._id)) {
          user.bookmarks.push(props.postData._id);
        }
        toast.success("Post saved!");
      } else {
        // Remove post from user's bookmarks
        if (user.bookmarks) {
          const index = user.bookmarks.indexOf(props.postData._id);
          if (index > -1) {
    ...
    <div class="p-4">
      <div class="flex items-center justify-between mb-3">
        ...
        <button
          ...
          class="hover:text-gray-900 transition-colors disabled:opacity-50"
        >
          <svg
            class="w-6 h-6"
